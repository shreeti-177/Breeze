...
//                });
    ...
    private void AddTotalExpenses(){
        m_ExpenseRef.child(String.valueOf(Util.getTransactionMonth(a_dateTime))).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
...
//            m_ExpenseRef.child(String.valueOf(Util.getTransactionMonth(a_dateTime))).child(a_Expense.getId()).setValue(a_Expense).addOnCompleteListener(task -> {
//                if (task.isSuccessful()) {
//                    Log.d(TAG, "AddTransaction: success");
////                    Toast.makeText(this, "Transaction added successfully", Toast.LENGTH_SHORT).show();
//                } else {
